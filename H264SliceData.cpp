 ...
 */
int CH264SliceData::slice_data(BitStream &bs, PictureBase &picture,
                               int32_t _slice_id) {
  int ret = 0;
  int32_t i = 0;
  CH264Golomb gb;
  ...
  // picNumLXPred is the prediction value for the variable picNumLXNoWrap. When
  // the process specified in this clause is invoked the first time for a slice
  // (that is, for the first occurrence of modification_of_pic_nums_idc equal to
  // 0 or 1 in the ref_pic_list_modification( ) syntax), picNumL0Pred and
  ...
  if (picture.m_slice_cnt == 0) {
    ...
    if (slice_header.m_sps.frame_mbs_only_flag == 0) // 有可能出现场帧或场宏块
    {
      picture.m_parent->m_picture_top_filed.copyDataPicOrderCnt(
          picture); // 顶（底）场帧有可能被选为参考帧，在解码P/B帧时，会用到PicOrderCnt字段，所以需要在此处复制一份
  ...
  do {
    if (slice_header.slice_type != H264_SLIECE_TYPE_I &&
      ...
      if (!slice_header.m_pps.entropy_coding_mode_flag) {
        ...
        for (i = 0; i < mb_skip_run; i++) {
          ...
          //-----------------------------------------------------------------
          ret = picture.m_mbs[picture.CurrMbAddr].macroblock_layer_mb_skip(
    ...
    if (moreDataFlag) {
      ...
      if (slice_header.MbaffFrameFlag &&
          (CurrMbAddr % 2 == 0 ||
           (CurrMbAddr % 2 == 1 &&
            prevMbSkipped))) // 表示本宏块是属于一个宏块对中的一个
      {
      ...
      //--------熵解码------------
      ret = picture.m_mbs[picture.CurrMbAddr].macroblock_layer(
          bs, picture, *this, cabac); // 2 | 3 | 4
      if (ret != 0) {
        printf("picture.m_mbs[%d].macroblock_layer(...) failed!\n",
               picture.CurrMbAddr);
      }
          ...
          Intra_4x4) // 帧内预测
      {
        ...
        ret = picture.transform_decoding_process_for_4x4_luma_residual_blocks(
            isChroma, isChromaCb, BitDepth, picWidthInSamplesL, pic_buff_luma);
        RETURN_IF_FAILED(ret != 0, ret);

        isChromaCb = 1;
        ret = picture.transform_decoding_process_for_chroma_samples(
                 ...
                 Intra_8x8) // 帧内预测
      {
        ...
        ret = picture.transform_decoding_process_for_8x8_luma_residual_blocks(
          ...
          I_PCM) // 说明该宏块没有残差，也没有预测值，码流中的数据直接为原始像素值
      {
        ...
        RETURN_IF_FAILED(ret != 0, ret);
             ...
             // (IS_INTER_Prediction_Mode(picture.m_mbs[picture.CurrMbAddr].m_mb_pred_mode))
             // //帧间预测
      {
        // The inter prediction process for P and B macroblocks is specified in
        // clause 8.4 with inter prediction samples being the output.
        ret = picture.Inter_prediction_process(); // 帧间预测
        RETURN_IF_FAILED(ret != 0, ret);
        ...
        if (picture.m_mbs[picture.CurrMbAddr].transform_size_8x8_flag == 0) {
          ret =
              picture
                  .transform_decoding_process_for_4x4_luma_residual_blocks_inter(
                      isChroma, isChromaCb, BitDepth, picWidthInSamplesL,
                      pic_buff_luma);
          RETURN_IF_FAILED(ret != 0, ret);
        } else // if (picture.m_mbs[picture.CurrMbAddr].transform_size_8x8_flag
               // == 1)
        {
          ret =
              picture
                  .transform_decoding_process_for_8x8_luma_residual_blocks_inter(
                      isChroma, isChromaCb, BitDepth, picWidthInSamplesL,
                      picture.m_mbs[picture.CurrMbAddr].LumaLevel8x8,
                      pic_buff_luma);
          RETURN_IF_FAILED(ret != 0, ret);
        }

        isChromaCb = 1;
        ret = picture.transform_decoding_process_for_chroma_samples_inter(
            isChromaCb, picWidthInSamplesC, pic_buff_cb);
        RETURN_IF_FAILED(ret != 0, ret);
  ...
  if (picture.mb_cnt == picture.PicSizeInMbs) {
    ...
    printf("picture.mb_cnt(%d) != picture.PicSizeInMbs(%d)\n", picture.mb_cnt,
           picture.PicSizeInMbs);
  }
