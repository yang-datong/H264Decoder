 ...
 * Golomb解码
 */
int CH264Golomb::get_ue_golomb(BitStream &bs) {
  ...
  int codeNum = (1 << leadingZeroBits) - 1 + bs.readUn(leadingZeroBits);
 ...
 */
int CH264Golomb::get_se_golomb(BitStream &bs) {
 ...
 * macroblock prediction modes (a) ChromaArrayType is equal to 1 or 2 codeNum
 * coded_block_pattern Intra_4x4, Intra_8x8     Inter 0            47 0 1 31 16
 *    2            15                     1
 *    3            0                      2
 *    4            23                     4
 *    5            27                     8
...
maping_exp_golomb_t maping_exp_golomb_arrays2[] = {
    {0, 15, 0},  {1, 0, 1},   {2, 7, 2},   {3, 11, 4},  {4, 13, 8}, {5, 14, 3},
    {6, 3, 5},   {7, 5, 10},  {8, 10, 12}, {9, 12, 15}, {10, 1, 7}, {11, 2, 11},
    {12, 4, 13}, {13, 8, 14}, {14, 6, 6},  {15, 9, 9},
};

int CH264Golomb::get_me_golomb(BitStream &bs, int32_t ChromaArrayType,
 ...
 *    equal to the value of the syntax element is given by a process equivalent
 * to: b = read_bits( 1 ) codeNum = !b
 *
 * Rec. ITU-T H.264 (04/2017) P209
 *
 * 截断指数Golomb熵编码
 * 参数 range
 * 的取值是根据相应的h264语法元素的取值范围来确定的，比如语法元素ref_idx_l0[]
 * 1. 当RefPicList0Length=1时，ref_idx_l0只能是0，此时输入参数range应该设为0
 * 2.
 * 当RefPicList0Length=2时，ref_idx_l0只能是0或者1，此时输入参数range应该设为1
 * 3.
 * 当RefPicList0Length>=3时，ref_idx_l0有更多的取值情况，此时输入参数range应该设为>2
 */
int CH264Golomb::get_te_golomb(BitStream &bs, int range) {
  int32_t codeNum = 0;
