...
int CH264VUI::vui_parameters(BitStream &bs) {
  ...
  if (this->aspect_ratio_info_present_flag) {
    this->aspect_ratio_idc = bs.readUn(8); // 0 u(8)
    if (this->aspect_ratio_idc == Extended_SAR) {
      this->sar_width = bs.readUn(16);  // 0 u(16)
      this->sar_height = bs.readUn(16); // 0 u(16)
    }
  }

  this->overscan_info_present_flag = bs.readUn(1); // 0 u(1)
  if (this->overscan_info_present_flag) {
    this->overscan_appropriate_flag = bs.readUn(1); // 0 u(1)
  }
  this->video_signal_type_present_flag = bs.readUn(1); // 0 u(1)
  if (this->video_signal_type_present_flag) {
    this->video_format = bs.readUn(3);                    // 0 u(3)
    this->video_full_range_flag = bs.readUn(1);           // 0 u(1)
    this->colour_description_present_flag = bs.readUn(1); // 0 u(1)
    if (this->colour_description_present_flag) {
      this->colour_primaries = bs.readUn(8);         // 0 u(8)
      this->transfer_characteristics = bs.readUn(8); // 0 u(8)
      this->matrix_coefficients = bs.readUn(8);      // 0 u(8)
    }
  }
  this->chroma_loc_info_present_flag = bs.readUn(1); // 0 u(1)
  ...
  this->timing_info_present_flag = bs.readUn(1); // 0 u(1)
  if (this->timing_info_present_flag) {
    this->num_units_in_tick = bs.readUn(32);    // 0 u(32)
    this->time_scale = bs.readUn(32);           // 0 u(32)
    this->fixed_frame_rate_flag = bs.readUn(1); // 0 u(1)
  }
  ...
  if (this->nal_hrd_parameters_present_flag ||
    ...
    this->low_delay_hrd_flag = bs.readUn(1); // 0 u(1)
  }
  this->pic_struct_present_flag = bs.readUn(1);    // 0 u(1)
  this->bitstream_restriction_flag = bs.readUn(1); // 0 u(1)
  if (this->bitstream_restriction_flag) {
    this->motion_vectors_over_pic_boundaries_flag = bs.readUn(1); // 0 u(1)
