...
int CH264PPS::copy(const CH264PPS &pps) {
  ...
  if (pps.slice_group_id && pps.pic_size_in_map_units_minus1 >= 0) {
    this->slice_group_id =
        (int32_t *)my_malloc(pps.pic_size_in_map_units_minus1 + 1);
    RETURN_IF_FAILED(this->slice_group_id == NULL, -1);
    memcpy(this->slice_group_id, pps.slice_group_id,
           sizeof(int32_t) * pps.pic_size_in_map_units_minus1 + 1);
  }

  return 0;
}

int CH264PPS::pic_parameter_set_rbsp(BitStream &bs, CH264SPS (&m_spss)[32]) {
  int ret = 0;
  ...
  entropy_coding_mode_flag = bs.readUn(1);                     // 1 u(1)
  bottom_field_pic_order_in_frame_present_flag = bs.readUn(1); // 1 u(1)
  ...
  if (num_slice_groups_minus1 > 0) {
    ...
    if (slice_group_map_type == 0) {
      ...
      slice_group_change_direction_flag = bs.readUn(1);      // 1 u(1)
      slice_group_change_rate_minus1 = gb.get_ue_golomb(bs); // 1 ue(v)
    } else if (slice_group_map_type == 6) {
      pic_size_in_map_units_minus1 = gb.get_ue_golomb(bs); // 1 ue(v)
      slice_group_id =
          (int32_t *)my_malloc(pic_size_in_map_units_minus1 +
                               1); // PicSizeInMapUnits − 1 = PicWidthInMbs *
                                   // PicHeightInMapUnits - 1;
      int v = h264_log2(num_slice_groups_minus1 +
                        1); // Ceil( Log2( num_slice_groups_minus1 + 1 ) );
      for (i = 0; i <= pic_size_in_map_units_minus1; i++) {
        slice_group_id[i] =
            bs.readUn(v); // 1 u(v)    v = Ceil( Log2( num_slice_groups_minus1
                          // + 1 ) ) bits.
      }
    }
  }
  ...
  weighted_pred_flag = bs.readUn(1);                           // 1 u(1)
  weighted_bipred_idc = bs.readUn(2);                          // 1 u(2)
  ...
  deblocking_filter_control_present_flag = bs.readUn(1); // 1 u(1)
  constrained_intra_pred_flag = bs.readUn(1);            // 1 u(1)
  redundant_pic_cnt_present_flag = bs.readUn(1);         // 1 u(1)

  second_chroma_qp_index_offset =
      chroma_qp_index_offset; // When second_chroma_qp_index_offset is not
                              // present, it shall be inferred to be equal to
                              // chroma_qp_index_offset.

  // if (more_rbsp_data(bs)) {
  // transform_8x8_mode_flag = bs.readUn(1);         // 1 u(1)
  // pic_scaling_matrix_present_flag = bs.readUn(1); // 1 u(1)
  // if (pic_scaling_matrix_present_flag) {
  // for (i = 0;
  // i <
  // 6 + ((m_spss[seq_parameter_set_id].chroma_format_idc != 3) ? 2 : 6) *
  // transform_8x8_mode_flag;
  // i++) {
  // pic_scaling_list_present_flag[i] = bs.readUn(1); // 1 u(1)
