...
class MacroBlock {
 public:
  ...
  int32_t
      CodedBlockPatternChroma; // CodedBlockPatternLuma = coded_block_pattern %
                               // 16; CodedBlockPatternChroma =
                               // coded_block_pattern / 16;
  int32_t
      CodedBlockPatternLuma; // specifies which of the four 8x8 luma blocks and
                             // associated chroma blocks of a macroblock may
                             // contain non-zero transform coefficient levels.
  int32_t QPY;  // QPY = ( ( QPY_prev + mb_qp_delta + 52 + 2 * QpBdOffsetY ) % (
                // 52 + QpBdOffsetY ) ) − QpBdOffsetY (7-37)
  ...
  int32_t QP1Cr; // QP′C
  int32_t
      QSCb; // When the current slice is an SP or SI slice, QSC is derived using
            // the above process, substituting QPY with QSY and QPC with QSC.
  int32_t QS1Cb; // QS′C
  int32_t QSCr;
  int32_t QS1Cr; // QS′C
  ...
  int32_t mb_field_decoding_flag; // 0-a frame macroblock pair; 1-a field
                                  // macroblock pair;
  int32_t MbaffFrameFlag; // MbaffFrameFlag = ( sps.mb_adaptive_frame_field_flag
                          // && !field_pic_flag );
  ...
  uint8_t
      coded_block_flag_DC_pattern; // 3个bit位表示CABAC残差相应4x4子宏块中的DC直流系数block的coded_block_flag值，(b7,...,b2,b1,b0)=(x,...,cr,cb,luma)
  uint16_t coded_block_flag_AC_pattern
      [3]; // 16个bit位表示CABAC残差相应4x4子宏块中的AC交流系数block的coded_block_flag值(0或1)(全部默认为1)，[0]-luma,[1]-cb,[2]-cr
 ...
 public:
  ...
  //  int macroblock_layer(PictureBase &picture, const SliceData &slice_data,
  //                       Cabac &cabac);
  //  int macroblock_layer_mb_skip(PictureBase &picture,
  //                               const SliceData &slice_data, Cabac &cabac);
  //  int macroblock_layer_mb_skip(PictureBase &picture, Cabac &cabac);
  //  int mb_pred(CBitstream &bs, PictureBase &picture, const SliceData
  //  &slice_data,
  //              Cabac &cabac);
  //  int sub_mb_pred(CBitstream &bs, PictureBase &picture,
  //                  const SliceData &slice_data, Cabac &cabac);
  //
  //  int residual(CBitstream &bs, PictureBase &picture, int32_t startIdx,
  //               int32_t endIdx, Cabac &cabac);
  //  int residual_luma(CBitstream &bs, PictureBase &picture,
  ...
  //                    MB_RESIDUAL_LEVEL mb_residual_level_ac, Cabac &cabac);
};
