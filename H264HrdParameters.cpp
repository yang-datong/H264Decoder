...
int CHrdParameters::hrd_parameters(BitStream &bs) {
  ...
  this->bit_rate_scale = bs.readUn(4);         // 0 | 5 u(4)
  this->cpb_size_scale = bs.readUn(4);         // 0 | 5 u(4)
  for (int32_t SchedSelIdx = 0; SchedSelIdx <= this->cpb_cnt_minus1;
       SchedSelIdx++) {
    this->bit_rate_value_minus1[SchedSelIdx] =
        gb.get_ue_golomb(bs); // 0 | 5 ue(v)
    this->cpb_size_value_minus1[SchedSelIdx] =
        gb.get_ue_golomb(bs);                   // 0 | 5 ue(v)
    this->cbr_flag[SchedSelIdx] = bs.readUn(1); // 0 | 5 u(1)
  }
  this->initial_cpb_removal_delay_length_minus1 = bs.readUn(5); // 0 | 5 u(5)
  this->cpb_removal_delay_length_minus1 = bs.readUn(5);         // 0 | 5 u(5)
  this->dpb_output_delay_length_minus1 = bs.readUn(5);          // 0 | 5 u(5)
  this->time_offset_length = bs.readUn(5);                      // 0 | 5 u(5)
