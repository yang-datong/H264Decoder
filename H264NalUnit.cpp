...
CH264NalUnit::CH264NalUnit() { int ret = init(0); }

CH264NalUnit::~CH264NalUnit() { int ret = unInit(); }
...
int CH264NalUnit::getH264RbspFromNalUnit(unsigned char *srcData, int srcSize) {
  ...
  int32_t temp = 0;
  BitStream bs(srcData, srcSize);

  forbidden_zero_bit = bs.readUn(1); // forbidden_zero_bit All f(1)
  nal_ref_idc = bs.readUn(2);        // nal_ref_idc All u(2)
  nal_unit_type = bs.readUn(5);      // nal_unit_type All u(5)
  ...
  if (nal_unit_type == 14    // H264_NAL_PREFIX
    ...
    {
      svc_extension_flag = bs.readUn(1); // svc_extension_flag All u(1)
    } else {
      avc_3d_extension_flag = bs.readUn(1); // avc_3d_extension_flag All u(1)
    }
    ...
    if (svc_extension_flag) {
      ...
      int32_t idr_flag = bs.readUn(1);                 // All u(1)
      int32_t priority_id = bs.readUn(6);              // All u(6)
      int32_t no_inter_layer_pred_flag = bs.readUn(1); // All u(1)
      int32_t dependency_id = bs.readUn(3);            // All u(3)
      int32_t quality_id = bs.readUn(4);               // All u(4)
      int32_t temporal_id = bs.readUn(3);              // All u(3)
      int32_t use_ref_base_pic_flag = bs.readUn(1);    // All u(1)
      int32_t discardable_flag = bs.readUn(1);         // All u(1)
      int32_t output_flag = bs.readUn(1);              // All u(1)
      int32_t reserved_three_2bits = bs.readUn(2);     // All u(2)
    } else if (avc_3d_extension_flag) {
      // nal_unit_header_3davc_extension() /* specified in Annex J */
      nalUnitHeaderBytes += 2;

      // J.7.3.1.1 NAL unit header 3D-AVC extension syntax
      int32_t view_idx = bs.readUn(8);        // All u(8)
      int32_t depth_flag = bs.readUn(1);      // All u(1)
      int32_t non_idr_flag = bs.readUn(1);    // All u(1)
      int32_t temporal_id = bs.readUn(3);     // All u(3)
      int32_t anchor_pic_flag = bs.readUn(1); // All u(1)
      int32_t inter_view_flag = bs.readUn(1); // All u(1)
    } else {
      ...
      int32_t non_idr_flag = bs.readUn(1);     // All u(1)
      int32_t priority_id = bs.readUn(6);      // All u(6)
      int32_t view_id = bs.readUn(10);         // All u(10)
      int32_t temporal_id = bs.readUn(3);      // All u(3)
      int32_t anchor_pic_flag = bs.readUn(1);  // All u(1)
      int32_t inter_view_flag = bs.readUn(1);  // All u(1)
      int32_t reserved_one_bit = bs.readUn(1); // All u(1)
    }
  }
  ...
  for (int32_t i = nalUnitHeaderBytes; i < NumBytesInNALunit; ++i) {
    // temp = bs.getBits(24);
    // TODO  <24-07-29 15:20:37, YangJing>

    if (i + 2 < NumBytesInNALunit && temp == 0x000003) {
      rbsp_byte[NumBytesInRBSP++] =
          bs.readUn(8); // rbsp_byte[ NumBytesInRBSP++ ] All b(8)
      rbsp_byte[NumBytesInRBSP++] =
          bs.readUn(8); // rbsp_byte[ NumBytesInRBSP++ ] All b(8)
      i += 2;
      emulation_prevention_three_byte = bs.readUn(
          8); // emulation_prevention_three_byte /* equal to 0x03 */ All f(8)
    } else {
      rbsp_byte[NumBytesInRBSP++] =
          bs.readUn(8); // rbsp_byte[ NumBytesInRBSP++ ] All b(8)
    }
