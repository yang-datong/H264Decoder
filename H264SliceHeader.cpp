...
//
//   copyData(src,
//            isMallocAndCopyData); // 重载的等号运算符，默认不my_malloc数据
//
//   return *this;
// }
//
// int CH264SliceHeader::copyData(const CH264SliceHeader &src,
//                                bool isCopyMallocData) {
//   if (isCopyMallocData) {
//     SAFE_FREE(this->mapUnitToSliceGroupMap);
//     SAFE_FREE(this->MbToSliceGroupMap);
//
...
//
//     this->mapUnitToSliceGroupMap =
//         (int32_t *)my_malloc(sizeof(int32_t) * src.m_sps.PicSizeInMapUnits);
//     RETURN_IF_FAILED(this->mapUnitToSliceGroupMap == NULL, -1);
//     memcpy(this->mapUnitToSliceGroupMap, src.mapUnitToSliceGroupMap,
//            sizeof(int32_t) * src.m_sps.PicSizeInMapUnits);
//
//     this->MbToSliceGroupMap =
//         (int32_t *)my_malloc(sizeof(int32_t) * src.PicSizeInMbs);
//     RETURN_IF_FAILED(this->MbToSliceGroupMap == NULL, -2);
//     memcpy(this->MbToSliceGroupMap, src.MbToSliceGroupMap,
//            sizeof(int32_t) * src.PicSizeInMbs);
//
...
//   disable_deblocking_filter_idc); printf("slice_alpha_c0_offset_div2=%d;\n",
//   slice_alpha_c0_offset_div2); printf("slice_beta_offset_div2=%d;\n",
//   slice_beta_offset_div2); printf("slice_group_change_cycle=%d;\n",
...
//   //    abs_diff_pic_num_minus1); printf("long_term_pic_num=%d;\n",
//   //    long_term_pic_num);
...
// }
//
// int CH264SliceHeader::slice_header(BitStream &bs, const CH264NalUnit
// &nal_unit,
//                                    const CH264SPS spss[32],
//                                    const CH264PPS ppss[256]) {
//   int ret = 0;
//   CH264Golomb gb;
//
...
//   }
//
//   frame_num = bs.readUn(m_sps.log2_max_frame_num_minus4 + 4); // 2 u(v)
...
//     pic_order_cnt_lsb = bs.readUn(v); // 2 u(v)
...
//     direct_spatial_mv_pred_flag = bs.readU1(); // 2 u(1)
//   }
//
//   if (slice_type == H264_SLIECE_TYPE_P || slice_type == H264_SLIECE_TYPE_SP
//   ||
//       slice_type == H264_SLIECE_TYPE_B) {
//     num_ref_idx_active_override_flag = bs.readU1(); // 2 u(1)
//
//     num_ref_idx_l0_active_minus1 =
//     m_pps.num_ref_idx_l0_default_active_minus1; num_ref_idx_l1_active_minus1
...
//     // ret = ref_pic_list_mvc_modification(bs); // /* specified in Annex H */
//     2 printf("Unsupported m_nal_unit.nal_unit_type == 20 || "
//            "m_nal_unit.nal_unit_type == 21;\n");
//     return -1;
//   } else {
...
//   }
//
//   if ((m_pps.weighted_pred_flag && (slice_type == H264_SLIECE_TYPE_P ||
//                                     slice_type == H264_SLIECE_TYPE_SP)) ||
...
//   }
//
//   if (m_pps.entropy_coding_mode_flag && slice_type != H264_SLIECE_TYPE_I &&
//       slice_type != H264_SLIECE_TYPE_SI) {
...
//       sp_for_switch_flag = bs.readU1(); // 2 u(1)
//     }
...
//   MapUnitsInSliceGroup0 = MIN(slice_group_change_cycle *
//   SliceGroupChangeRate,
...
//         (int32_t *)my_malloc(sizeof(int32_t) * m_sps.PicSizeInMapUnits);
//     RETURN_IF_FAILED(mapUnitToSliceGroupMap == NULL, -1);
//     memset(mapUnitToSliceGroupMap, 0,
//            sizeof(int32_t) * m_sps.PicSizeInMapUnits);
//   }
//
...
// */
// int CH264SliceHeader::ref_pic_list_modification(BitStream &bs) {
//  CH264Golomb gb;
//
//  if (slice_type % 5 != 2 &&
//      slice_type % 5 != 4) // H264_SLIECE_TYPE_I = 2; H264_SLIECE_TYPE_SI = 4;
//  {
//    ref_pic_list_modification_flag_l0 = bs.readU1(); // 2 u(1)
//    if (ref_pic_list_modification_flag_l0) {
//      int32_t i = 0;
//      do {
//        if (i >= 32) {
//          printf(
//              "modification_of_pic_nums_idc[0][i]: i=%d, must be in [0,31]\n",
//              i);
//          return -1;
//        }
//
...
//  {
//    ref_pic_list_modification_flag_l1 = bs.readU1(); // 2 u(1)
//    if (ref_pic_list_modification_flag_l1) {
//      int32_t i = 0;
//      do {
//        if (i >= 32) {
//          printf(
//              "modification_of_pic_nums_idc[1][i]: i=%d, must be in [0,31]\n",
//              i);
//          return -1;
//        }
//
...
// */
// int CH264SliceHeader::pred_weight_table(BitStream &bs) {
//  int32_t i = 0;
//  int32_t j = 0;
//  CH264Golomb gb;
//
...
//    luma_weight_l0[i] =
//        1 << luma_log2_weight_denom; // When luma_weight_l0_flag is equal to 0
//    luma_offset_l0[i] = 0;           // When luma_weight_l0_flag is equal to 0
//
//    luma_weight_l0_flag = bs.readU1(); // 2 u(1)
...
//
//    if (m_sps.ChromaArrayType != 0) {
//      chroma_weight_l0[i][0] =
//          1 << chroma_log2_weight_denom; // When chroma_weight_l0_flag is
//          equal
//                                         // to 0
//      chroma_weight_l0[i][1] =
//          1 << chroma_log2_weight_denom; // When chroma_weight_l0_flag is
//          equal
//                                         // to 0
...
//
//      chroma_weight_l0_flag = bs.readU1(); // 2 u(1)
...
//      luma_weight_l1[i] =
//          1 << luma_log2_weight_denom; // When luma_weight_l1_flag is equal to
//          0
//      luma_offset_l1[i] = 0;           // When luma_weight_l1_flag is equal to
//      0
//
//      luma_weight_l1_flag = bs.readU1(); // 2 u(1)
...
//
//      if (m_sps.ChromaArrayType != 0) {
//        chroma_weight_l1[i][0] =
//            1 << chroma_log2_weight_denom; // When chroma_weight_l1_flag is
//                                           // equal to 0
//        chroma_weight_l1[i][1] =
//            1 << chroma_log2_weight_denom; // When chroma_weight_l1_flag is
//                                           // equal to 0
//        chroma_offset_l1[i][0] = 0;
//        chroma_offset_l1[i][1] = 0;
//
//        chroma_weight_l1_flag = bs.readU1(); // 2 u(1)
...
// */
// int CH264SliceHeader::dec_ref_pic_marking(BitStream &bs) {
//  CH264Golomb gb;
//
//  if (m_nal_unit.IdrPicFlag) {
//    no_output_of_prior_pics_flag = bs.readU1(); // 2 | 5 u(1)
//    long_term_reference_flag = bs.readU1();     // 2 | 5 u(1)
//  } else {
//    adaptive_ref_pic_marking_mode_flag = bs.readU1(); // 2 | 5 u(1)
//    if (adaptive_ref_pic_marking_mode_flag) {
//      int32_t i = 0;
//      do {
//        if (i >= 32) {
//          printf("%s: m_dec_ref_pic_marking[i]: i=%d, must be in [0,31]\n",
//                 __FUNCTION__, i);
//          break;
//        }
//
//        m_dec_ref_pic_marking[i].memory_management_control_operation =
//            gb.get_ue_golomb(bs); // 2 | 5 ue(v)
...
//            {
//          m_dec_ref_pic_marking[i].difference_of_pic_nums_minus1 =
//              gb.get_ue_golomb(bs); // 2 | 5 ue(v)
//        }
//        if (m_dec_ref_pic_marking[i].memory_management_control_operation == 2)
//        {
//          m_dec_ref_pic_marking[i].long_term_pic_num_2 =
//              gb.get_ue_golomb(bs); // 2 | 5 ue(v)
//        }
...
//            {
//          m_dec_ref_pic_marking[i].long_term_frame_idx =
//              gb.get_ue_golomb(bs); // 2 | 5 ue(v)
//        }
//        if (m_dec_ref_pic_marking[i].memory_management_control_operation == 4)
//        {
//          m_dec_ref_pic_marking[i].max_long_term_frame_idx_plus1 =
//              gb.get_ue_golomb(bs); // 2 | 5 ue(v)
//        }
//        i++;
//      } while (
...
//     } else // if (m_sps.frame_mbs_only_flag == 0 &&
//            // m_sps.mb_adaptive_frame_field_flag == 0 && field_pic_flag == 0)
//     {
//       MbToSliceGroupMap[i] =
...
//       0) // 8.2.2.1 Specification for interleaved slice group map type 交叉型
//          // slice组映射类型的描述
//   {
//     i = 0;
//     do {
...
//       int32_t yBottomRight = m_pps.bottom_right[iGroup] /
//       m_sps.PicWidthInMbs; int32_t xBottomRight = m_pps.bottom_right[iGroup]
//       % m_sps.PicWidthInMbs; for (y = yTopLeft; y <= yBottomRight; y++) {
...
//     }
//   } else if (m_pps.slice_group_map_type ==
//              3) // 8.2.2.4 Specification for box-out slice group map types
...
//     }
//   } else if (m_pps.slice_group_map_type ==
//              4) // 8.2.2.5 Specification for raster scan slice group map
//              types
...
//     }
//   } else if (m_pps.slice_group_map_type ==
//              5) // 8.2.2.6 Specification for wipe slice group map types
//              擦除型
//                 // slice 组映射类型的描述
//   {
//     int32_t sizeOfUpperLeftGroup = 0;
...
//     }
//   } else {
//     printf("m_pps.slice_group_map_type=%d, must be in [0..6];\n",
...
// }
//
// int CH264SliceHeader::set_scaling_lists_values(const CH264SPS &sps,
//                                                const CH264PPS &pps) {
//   int ret = 0;
//
//   //--------------------------------
...
//               }
//             } else {
//               memcpy(ScalingList4x4[i], sps.ScalingList4x4[i],
//                      sizeof(int32_t) *
...
//               }
//             } else {
//               memcpy(ScalingList8x8[i - 6], sps.ScalingList8x8[i - 6],
//                      sizeof(int32_t) *
...
// }
//
// bool CH264SliceHeader::is_first_VCL_NAL_unit_of_a_picture(
//     const CH264SliceHeader &lastSliceHeader) {
//   int ret = 0;
//
//   ret |= (this->pic_parameter_set_id !=
//   lastSliceHeader.pic_parameter_set_id);
//
...
//   }
//
//   ret |= (this->m_nal_unit.nal_ref_idc !=
//           lastSliceHeader.m_nal_unit.nal_ref_idc) &&
//          ((this->m_nal_unit.nal_ref_idc == 0) ||
//           (lastSliceHeader.m_nal_unit.nal_ref_idc == 0));
//   ret |= (this->m_nal_unit.IdrPicFlag !=
//   lastSliceHeader.m_nal_unit.IdrPicFlag);
//
...
//     if (this->m_pps.bottom_field_pic_order_in_frame_present_flag == 1 &&
//         this->field_pic_flag == 0) {
//       ret |= (this->delta_pic_order_cnt_bottom !=
//               lastSliceHeader.delta_pic_order_cnt_bottom);
//     }
//   }
//
//   if (this->m_sps.pic_order_cnt_type == 1) {
//     if (!this->m_sps.delta_pic_order_always_zero_flag) {
//       ret |= (this->delta_pic_order_cnt[0] !=
//               lastSliceHeader.delta_pic_order_cnt[0]);
//       if (this->m_pps.bottom_field_pic_order_in_frame_present_flag == 1 &&
//           this->field_pic_flag == 0) {
//         ret |= (this->delta_pic_order_cnt[1] !=
//                 lastSliceHeader.delta_pic_order_cnt[1]);
//       }
//     }
//   }
//
