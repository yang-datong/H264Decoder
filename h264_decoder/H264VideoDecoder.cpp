...
int CH264VideoDecoder::init() { return 0; }

int CH264VideoDecoder::unInit() { return 0; }

int CH264VideoDecoder::set_output_frame_callback_functuin(
    output_frame_callback output_frame_callback, void *userData) {
...
int CH264VideoDecoder::open(const char *url) {
  ...
  picture_current =
      pictures_gop->m_DecodedPictureBuffer[0]; // 初始化第一个picture_current
  ...
  while (1) {
    ...
    //    static int nul = 0;
    //    printf("\tNAL[%d]{%d %d %d %d},Buffer len[%d]\n", ++nul, data[0],
    //    data[1],
    //           data[2], data[3], size);
    ...
    printf("\tRBSP[%d]{%d %d %d %d},Buffer len[%d]\n", ++rbsp, nu.rbsp_byte[0],
           nu.rbsp_byte[1], nu.rbsp_byte[2], nu.rbsp_byte[3],
           nu.NumBytesInRBSP);
    ...
    case H264_NAL_SLICE: // 1
    {
      ...
      ret = slice_header.slice_header(bs, nu, pictures_gop->m_spss,
                                      pictures_gop->m_ppss); // 2
      RETURN_IF_FAILED(ret != 0, ret);

      bool is_new_pic =
          slice_header.is_first_VCL_NAL_unit_of_a_picture(slice_header_last);
      if (is_new_pic == true && picture_current != NULL &&
          picture_current->m_current_picture_ptr != NULL) {
        CH264Picture *newEmptyPicture = NULL;
        picture_current->m_current_picture_ptr
            ->end_decode_the_picture_and_get_a_new_empty_picture(
                newEmptyPicture);
        RETURN_IF_FAILED(ret != 0, ret);

        is_need_flush =
            picture_current->m_picture_frame.m_h264_slice_header.m_nal_unit
                .IdrPicFlag; // 当前已解码完毕的帧是否是IDR帧
        ret = do_callback(picture_current, pictures_gop,
                          is_need_flush); // 回调操作
        if (ret != 0) {
      ...
      slice_header.syntax_element_categories =
          (0x01 | 0x02) | 0x04; // 2 | 3 | 4;
      slice_header.slice_id = 0;
      ret = picture_current->decode_one_slice(
          slice_header, bs,
          pictures_gop
              ->m_DecodedPictureBuffer); // /* all categories of slice_data()
                                         // syntax */ 2 | 3 | 4
      CONTINUE_IF_FAILED(ret != 0);

      ret = rbsp_slice_trailing_bits(
          bs, slice_header.m_pps.entropy_coding_mode_flag); // 2
      RETURN_IF_FAILED(ret != 0, ret);
    ...
    case H264_NAL_DPA: // 2
    {
      ...
      ret = picture_current->decode_one_slice(
          slice_header, bs,
          pictures_gop->m_DecodedPictureBuffer); // /* only category 2 parts of
                                                 // slice_data() syntax */ 2
      CONTINUE_IF_FAILED(ret != 0);
    ...
    case H264_NAL_DPB: // 3
    {
      ...
      CH264SliceHeader &slice_header =
          picture_current->m_current_picture_ptr->m_h264_slice_header;
      CH264Golomb gb;
      ...
      ret = picture_current->decode_one_slice(
          slice_header, bs,
          pictures_gop->m_DecodedPictureBuffer); // /* only category 3 parts of
                                                 // slice_data() syntax */ 3
      CONTINUE_IF_FAILED(ret != 0);

      ret = rbsp_slice_trailing_bits(
          bs, slice_header.m_pps.entropy_coding_mode_flag); // 3
      RETURN_IF_FAILED(ret != 0, ret);
    ...
    case H264_NAL_DPC: // 4
    {
      ...
      ret = picture_current->decode_one_slice(
          slice_header, bs,
          pictures_gop->m_DecodedPictureBuffer); // /* only category 4 parts of
                                                 // slice_data() syntax */ 4
      CONTINUE_IF_FAILED(ret != 0);

      ret = rbsp_slice_trailing_bits(
          bs, slice_header.m_pps.entropy_coding_mode_flag); // 4
      RETURN_IF_FAILED(ret != 0, ret);
    ...
    case H264_NAL_IDR_SLICE: // 5 立即刷新帧（每个GOP的第一帧必须是I帧）
    {
      if (sps_flag == 0 || pps_flag == 0) {
        LOG_ERROR("For IDR slice, sps and pps should be present first. "
                  "m_sps_index=%d; m_pps_index=%d;\n",
                  sps_flag, pps_flag);
        break;
      }
    ...
    case H264_NAL_SPS: // 7
    {
      ...
      if (ret != 0) {
        LOG_ERROR("this->m_sps[m_sps_index].seq_parameter_set_data(bs) failed! "
                  "ret=%d;\n",
                  ret);
      }

      if (sps.seq_parameter_set_id < 0 ||
          sps.seq_parameter_set_id >= H264_MAX_SPS_COUNT) {
        LOG_ERROR("seq_parameter_set_id shall be in the range of 0 to 31, "
                  "inclusive. sps.seq_parameter_set_id=%d;\n",
                  sps.seq_parameter_set_id);
      }

      pictures_gop->m_spss[sps.seq_parameter_set_id] = sps;
      pictures_gop->max_num_reorder_frames =
          sps.m_vui.max_num_reorder_frames; // 用于按显示顺序输出B帧
      sps_flag = 1;
      break;
    }
    case H264_NAL_PPS: // 8
    {
      ...
      if (ret != 0) {
        LOG_ERROR("this->m_pps[m_pps_index].pic_parameter_set_rbsp(bs) failed! "
                  "ret=%d;\n",
                  ret);
      }

      if (pps.pic_parameter_set_id < 0 ||
          pps.pic_parameter_set_id >= H264_MAX_PPS_COUNT) {
        LOG_ERROR("pic_parameter_set_id shall be in the range of 0 to 255, "
                  "inclusive. pps.pic_parameter_set_id=%d;\n",
    ...
    case H264_NAL_SPS_EXT: // 13
    {
      ...
      if (ret != 0) {
        LOG_ERROR("pictures_gop->m_sps_ext.seq_parameter_set_extension_rbsp(bs)"
                  " failed! ret=%d;\n",
                  ret);
      }
      break;
    ...
    if (picture_current) {
      is_need_flush =
          picture_current->m_picture_frame.m_h264_slice_header.m_nal_unit
              .IdrPicFlag; // 当前已解码完毕的帧是否是IDR帧
      ret =
          do_callback(picture_current, pictures_gop, is_need_flush); // 回调操作
      // RETURN_IF_FAILED(ret != 0, ret);
    }
...
int CH264VideoDecoder::do_callback(CH264Picture *picture_current,
                                   CH264PicturesGOP *pictures_gop,
    ...
    while (1) {
      ...
      if (outPicture != NULL) {
        if (m_output_frame_callback != NULL) {
          ret = m_output_frame_callback(
              outPicture, m_userData,
