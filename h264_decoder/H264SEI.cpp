...
int CH264SEI::sei_payload(CBitstream &bs, int32_t payloadType,
                          int32_t payloadSize) {
  int ret = 0;
  /*
      if ( payloadType == 0 )
          ...
          three_dimensional_reference_displays_info( payloadSize ); // specified
     in Annex I else if ( payloadType == 52 ) depth_timing( payloadSize ); //
     specified in Annex I else if ( payloadType == 53 ) depth_sampling_info(
     payloadSize ); // specified in Annex I else if ( payloadType == 54 )
          constrained_depth_parameter_set_identifier( payloadSize ); //
     specified in Annex J else if ( payloadType == 56 ) green_metadata(
     payloadSize ); // specified in ISO/IEC 23001-11 else if ( payloadType ==
     137 ) mastering_display_colour_volume( payloadSize ); else if ( payloadType
     == 142 ) colour_remapping_info( payloadSize ); else if ( payloadType == 147
     ) alternative_transfer_characteristics( payloadSize ); else if (
     payloadType == 181 ) alternative_depth_info( payloadSize ); // specified in
     Annex I else reserved_sei_message( payloadSize );
  */
  if (!byte_aligned(bs)) {
