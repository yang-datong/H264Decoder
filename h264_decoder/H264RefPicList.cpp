...
int CH264PictureBase::Decoding_process_for_picture_order_count_type_0(
    const CH264PictureBase *picture_previous_ref) {
  ...
  if ((m_h264_slice_header.pic_order_cnt_lsb < prevPicOrderCntLsb) &&
      ((prevPicOrderCntLsb - m_h264_slice_header.pic_order_cnt_lsb) >=
       (m_h264_slice_header.m_sps.MaxPicOrderCntLsb / 2))) {
    PicOrderCntMsb =
        prevPicOrderCntMsb + m_h264_slice_header.m_sps.MaxPicOrderCntLsb;
  } else if ((m_h264_slice_header.pic_order_cnt_lsb > prevPicOrderCntLsb) &&
             ((m_h264_slice_header.pic_order_cnt_lsb - prevPicOrderCntLsb) >
              (m_h264_slice_header.m_sps.MaxPicOrderCntLsb / 2))) {
    PicOrderCntMsb =
        prevPicOrderCntMsb - m_h264_slice_header.m_sps.MaxPicOrderCntLsb;
  } else {
                                         ...
                                         // top field
  {
    ...
    {
      BottomFieldOrderCnt =
          TopFieldOrderCnt + m_h264_slice_header.delta_pic_order_cnt_bottom;
    } else // 当前图像为底场
    {
      BottomFieldOrderCnt =
          PicOrderCntMsb + m_h264_slice_header.pic_order_cnt_lsb;
    }
  }
...
int CH264PictureBase::Decoding_process_for_picture_order_count_type_1(
    const CH264PictureBase *picture_previous) {
  ...
  if (absFrameNum > 0) {
    picOrderCntCycleCnt =
        (absFrameNum - 1) /
        m_h264_slice_header.m_sps.num_ref_frames_in_pic_order_cnt_cycle;
  ...
  if (absFrameNum > 0) {
    expectedPicOrderCnt =
        picOrderCntCycleCnt *
        m_h264_slice_header.m_sps.ExpectedDeltaPerPicOrderCntCycle;
    for (int i = 0; i <= frameNumInPicOrderCntCycle; i++) {
      expectedPicOrderCnt = expectedPicOrderCnt +
                            m_h264_slice_header.m_sps.offset_for_ref_frame[i];
    }
  } else {
  ...
  {
    TopFieldOrderCnt =
        expectedPicOrderCnt + m_h264_slice_header.delta_pic_order_cnt[0];
    BottomFieldOrderCnt =
        TopFieldOrderCnt +
        m_h264_slice_header.m_sps.offset_for_top_to_bottom_field +
        m_h264_slice_header.delta_pic_order_cnt[1];
  ...
  } else // 当前图像为底场
  {
    BottomFieldOrderCnt =
        expectedPicOrderCnt +
        m_h264_slice_header.m_sps.offset_for_top_to_bottom_field +
        m_h264_slice_header.delta_pic_order_cnt[0];
  }
...
int CH264PictureBase::Decoding_process_for_picture_order_count_type_2(
    const CH264PictureBase *picture_previous) {
  RETURN_IF_FAILED(m_h264_slice_header.m_nal_unit.IdrPicFlag != 1 &&
                       picture_previous == NULL,
                   -1);
...
// SP, or B slice.
int CH264PictureBase::Decoding_process_for_reference_picture_lists_construction(
    CH264Picture *(&dpb)[16], CH264Picture *(&RefPicList0)[16],
    CH264Picture *(&RefPicList1)[16]) {
  int ret = 0;
...
// 8.2.4.1 Decoding process for picture numbers
int CH264PictureBase::Decoding_process_for_picture_numbers(
    CH264Picture *(&dpb)[16]) {
  int ret = 0;
  CH264SliceHeader &slice_header = m_h264_slice_header;
  int32_t size_dpb = 16;
    ...
    if (dpb[i]->m_picture_frame.reference_marked_type ==
      ...
      if (dpb[i]->m_picture_frame.FrameNum > slice_header.frame_num) {
        dpb[i]->m_picture_frame.FrameNumWrap =
            ...
            dpb[i]->m_picture_frame.m_h264_slice_header.m_sps.MaxFrameNum;
      } else {
    ...
    if (dpb[i]->m_picture_top_filed.reference_marked_type ==
      ...
      if (dpb[i]->m_picture_top_filed.FrameNum > slice_header.frame_num) {
        dpb[i]->m_picture_top_filed.FrameNumWrap =
            ...
            dpb[i]->m_picture_top_filed.m_h264_slice_header.m_sps.MaxFrameNum;
      } else {
        dpb[i]->m_picture_top_filed.FrameNumWrap =
            dpb[i]->m_picture_top_filed.FrameNum;
      }
    ...
    if (dpb[i]->m_picture_bottom_filed.reference_marked_type ==
      ...
      if (dpb[i]->m_picture_bottom_filed.FrameNum > slice_header.frame_num) {
        dpb[i]->m_picture_bottom_filed.FrameNumWrap =
            ...
            dpb[i]
                ->m_picture_bottom_filed.m_h264_slice_header.m_sps.MaxFrameNum;
      } else {
        dpb[i]->m_picture_bottom_filed.FrameNumWrap =
            dpb[i]->m_picture_bottom_filed.FrameNum;
      }
...
// header.
int CH264PictureBase::Initialisation_process_for_reference_picture_lists(
    CH264Picture *(&dpb)[16], CH264Picture *(&RefPicList0)[16],
    CH264Picture *(&RefPicList1)[16]) {
  int ret = 0;
  CH264SliceHeader &slice_header = m_h264_slice_header;
...
int CH264PictureBase::
    Initialisation_process_for_reference_picture_lists_for_B_slices_in_frames(
        CH264Picture *(&dpb)[16], CH264Picture *(&RefPicList0)[16],
        CH264Picture *(&RefPicList1)[16], int32_t &RefPicList0Length,
        int32_t &RefPicList1Length) {
  int ret = 0;
  int32_t size_dpb = 16;
  int32_t i = 0;
  int32_t j = 0;
...
// 8.2.4.2.5 Initialisation process for reference picture lists in fields
int CH264PictureBase::
    Initialisation_process_for_reference_picture_lists_in_fields(
        CH264Picture *(&refFrameListXShortTerm)[16],
        CH264Picture *(&refFrameListXLongTerm)[16],
        CH264Picture *(&RefPicListX)[16], int32_t &RefPicListXLength,
        int32_t listX) {
  int ret = 0;
  int32_t size = 16;
  int32_t i = 0;
...
// 参考图像列表的重排序过程
int CH264PictureBase::Modification_process_for_reference_picture_lists(
    CH264Picture *(&RefPicList0)[16], CH264Picture *(&RefPicList1)[16]) {
  int ret = 0;
  CH264SliceHeader &slice_header = m_h264_slice_header;
...
// 0.
int CH264PictureBase::Decoded_reference_picture_marking_process(
    CH264Picture *(&dpb)[16]) {
  int ret = 0;
  ret =
...
int CH264PictureBase::Decoding_process_for_gaps_in_frame_num() { return 0; }

// 8.2.5.3 Sliding window decoded reference picture marking process
// 滑动窗口解码参考图像的标识过程 This process is invoked when
// adaptive_ref_pic_marking_mode_flag is equal to 0.
int CH264PictureBase::Sliding_window_decoded_reference_picture_marking_process(
    CH264Picture *(&dpb)[16]) {
  CH264SliceHeader &slice_header = m_h264_slice_header;
                                                              ...
                                                              // reference"
  {
    ...
    if (numShortTerm + numLongTerm ==
            MAX(slice_header.m_sps.max_num_ref_frames, 1) &&
        numShortTerm > 0) {
