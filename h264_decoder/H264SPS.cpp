...
int CH264SPS::seq_parameter_set_data(CBitstream &bs) {
  ...
  this->reserved_zero_2bits =
      bs.readBits(2); // reserved_zero_2bits /* equal to 0 */ 0 u(2)
  ...
  if (profile_idc == 100    // A.2.4 High profile //Only I, P, and B slice types
    ...
    if (this->chroma_format_idc == 3) {
      this->separate_colour_plane_flag =
          bs.readBits(1); // separate_colour_plane_flag 0 u(1)
    }
    ...
    this->qpprime_y_zero_transform_bypass_flag =
        bs.readBits(1); // qpprime_y_zero_transform_bypass_flag 0 u(1)
    this->seq_scaling_matrix_present_flag =
        bs.readBits(1); // seq_scaling_matrix_present_flag 0 u(1)

    if (this->seq_scaling_matrix_present_flag) {
      for (i = 0; i < ((this->chroma_format_idc != 3) ? 8 : 12); i++) {
        this->seq_scaling_list_present_flag[i] =
            bs.readBits(1); // seq_scaling_list_present_flag[ i ] 0 u(1)
  ...
  if (this->pic_order_cnt_type == 0) {
    ...
    this->delta_pic_order_always_zero_flag =
        bs.readBits(1); // delta_pic_order_always_zero_flag 0 u(1)
  ...
  this->gaps_in_frame_num_value_allowed_flag =
      bs.readBits(1); // gaps_in_frame_num_value_allowed_flag 0 u(1)
  ...
  if (!this->frame_mbs_only_flag) {
    this->mb_adaptive_frame_field_flag =
        bs.readBits(1); // mb_adaptive_frame_field_flag 0 u(1)
  }

  this->direct_8x8_inference_flag =
      bs.readBits(1); // direct_8x8_inference_flag 0 u(1)
  ...
  this->vui_parameters_present_flag =
      bs.readBits(1); // vui_parameters_present_flag 0 u(1)
