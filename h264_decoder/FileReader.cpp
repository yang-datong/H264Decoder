...
CFileReader::~CFileReader() { int ret = unInit(); }
...
int CFileReader::getNextStartCode(unsigned char *data, int size,
                                  unsigned char *&pos1, unsigned char *&pos2,
                                  int &startCodeLength1,
                                  int &startCodeLength2) {
  int ret = -1;
...
int CFileReader::getNextH264NalUnitByStartCode(unsigned char *&data,
                                               int &size) {
  int ret = 0;
  ...
  while (1) {
    ...
    ret = getNextStartCode(m_buffer_pointer_pos,
                           m_buffer_end - m_buffer_pointer_pos + 1, pos1, pos2,
                           startCodeLength1, startCodeLength2);
    if (ret == 2) {
      // 在m_buffer_pointer_pos[0..end]中，从左到右找到了两个连续的起始码，则结束查找（即m_buffer_start[]数组必须能够容纳尺寸最大的I帧）
      ...
      // 在m_buffer_pointer_pos[0..end]中，从左到右只找到了第一个起始码
      // 将00 00 00
      // 01起始码以及之后的数据，向前移动，保持与m_buffer_start[0]对齐
      if (pos1 > m_buffer_pointer_pos) {
        // 类似 memmove(m_buffer_start, pos1, m_buffer_end - (pos1 -
        // m_buffer_start) + 1);
      ...
      int left_bytes =
          m_buffer_size - (m_buffer_end - m_buffer_pointer_pos + 2);
      if (left_bytes > 0) {
        size_t readBytes =
            fread(m_buffer_end + 1, 1, left_bytes, m_fp); // 从文件中读取数据
        ...
        LOG_ERROR("This nal unit is too large. 1111: m_buffer_size=%d; "
                  "m_file_offset=%d;\n",
    ...
    {
      m_buffer_end = m_buffer_start;
      LOG_ERROR("Cannot found start code [00 00 01]. 2222: m_buffer_size=%d; "
                "m_file_offset=%d;\n",
                m_buffer_size, m_file_offset);
      return -1;
    }
