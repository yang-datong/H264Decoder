...
// H264解码器，解码完一帧后的回调函数
int my_output_frame_callback(CH264Picture *outPicture, void *userData,
                             int errorCode) {
  int ret = 0;
  ...
  if (outPicture) {
    ...
    char filename[600] = {0};
    sprintf(
        filename, "%s/out_%dx%d.%d.bmp", outDir,
        outPicture->m_picture_frame.PicWidthInSamplesL,
        outPicture->m_picture_frame.PicHeightInSamplesL,
        s_PicNumCnt); // 不要用outPicture->m_picture_frame.m_PicNumCnt的值，因为对于含有B帧的视频来说，此值是解码顺序，不是帧显示顺序
...
int printHelp(int argc, char *argv[]) {
  ...
  printf("For Example:\n");
  printf("  %s ../data/HeavyHand_1080p.B_frames_cabac_tff.h264 "
         "../data/out_bmp_dir\n",
         argv[0]);
  return 0;
}
...
int main(int argc, char *argv[]) {
  ...
  char url[] = "./demo_10_frames.h264";
  char outDir[] = "./out";
  //------------------------
  CH264VideoDecoder vd;
  void *userData = outDir;

  ret = vd.set_output_frame_callback_functuin(my_output_frame_callback,
                                              userData); // 设置回调函数
