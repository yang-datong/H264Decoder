...
class CH264ResidualBlockCavlc {
 ...
 public:
  ...
  int residual_block_cavlc(int32_t *coeffLevel, int32_t startIdx,
                           ...
                           int32_t &TotalCoeff);
  int get_nC(MB_RESIDUAL_LEVEL mb_residual_level, int32_t MbPartPredMode,
  ...
  int coeff_token_table(int32_t nC, uint16_t coeff_token,
                        ...
                        int32_t &TotalCoeff);
  int get_total_zeros(int32_t maxNumCoeff, int32_t tzVlcIndex,
                      int32_t &total_zeros);
  int get_run_before(int32_t zerosLeft, int32_t &run_before);
};
