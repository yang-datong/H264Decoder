#ifndef CH264SLICEHEADER_HPP_SUJHV4QE
#define CH264SLICEHEADER_HPP_SUJHV4QE
...
class CH264SliceHeader {
 public:
  ...
  int32_t
      disable_deblocking_filter_idc; // 2 ue(v)
                                     // 0：开启环路滤波，滤波可跨越slice边界。1：关闭环路滤波。2：开启环路滤波，只针对同一个slice滤波。
  ...
  int32_t
      QPY_prev; // QPY,PREV 是当前 slice 中前一宏块的量化参数 QPY 的值，在每个
                ...
                // 应该被初始化成等式 7-16 中的 SliceQPY 值。
  int32_t SliceQPY; // SliceQPY = 26 + pic_init_qp_minus26 + slice_qp_delta;
  int32_t MbaffFrameFlag; // MbaffFrameFlag = ( mb_adaptive_frame_field_flag &&
                          // !field_pic_flag );
  int32_t PicHeightInMbs; // PicHeightInMbs = FrameHeightInMbs / ( 1 +
                          // field_pic_flag );
 ...
 public:
  ...
  bool
  is_first_VCL_NAL_unit_of_a_picture(const CH264SliceHeader &lastSliceHeader);
};

#endif /* end of include guard: CH264SLICEHEADER_HPP_SUJHV4QE */
