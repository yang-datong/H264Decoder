...
// and separate_colour_plane_flag
/*
...
CH264SPS::CH264SPS() {
  ...
  memset(offset_for_ref_frame, 0,
         sizeof(int32_t) * H264_MAX_OFFSET_REF_FRAME_COUNT);
...
int CH264SPS::seq_parameter_set_data(BitStream &bs) {
  int ret = 0;

  int32_t i = 0;
  CH264Golomb gb;

  this->profile_idc = bs.readUn(8);          // profile_idc 0 u(8)
  this->constraint_set0_flag = bs.readUn(1); // constraint_set0_flag 0 u(1)
  this->constraint_set1_flag = bs.readUn(1); // constraint_set1_flag 0 u(1)
  this->constraint_set2_flag = bs.readUn(1); // constraint_set2_flag 0 u(1)
  this->constraint_set3_flag = bs.readUn(1); // constraint_set3_flag 0 u(1)
  this->constraint_set4_flag = bs.readUn(1); // constraint_set4_flag 0 u(1)
  this->constraint_set5_flag = bs.readUn(1); // constraint_set5_flag 0 u(1)
  this->reserved_zero_2bits =
      bs.readUn(2);               // reserved_zero_2bits /* equal to 0 */ 0 u(2)
  this->level_idc = bs.readUn(8); // level_idc 0 u(8)
  this->seq_parameter_set_id =
      gb.get_ue_golomb(bs); // seq_parameter_set_id 0 ue(v)

  this->chroma_format_idc =
      1; // When chroma_format_idc is not present, it shall be inferred to be
         // equal to 1 (4:2:0 chroma format).
  int32_t profile_idc = this->profile_idc;

  // profile_idc == 66 //A.2.1 Baseline profile //Only I and P slice types may
  // be present. profile_idc == 77 //A.2.2 Main profile //Only I, P, and B slice
  // types may be present. profile_idc == 88 //A.2.3 Extended profile

  if (profile_idc == 100    // A.2.4 High profile //Only I, P, and B slice types
                            // may be present.
      || profile_idc == 110 // A.2.5(A.2.8) High 10 (Intra) profile //Only I, P,
                            // and B slice types may be present.
      || profile_idc == 122 // A.2.6(A.2.9) High 4:2:2 (Intra) profile //Only I,
                            // P, and B slice types may be present.
      || profile_idc == 244 // A.2.7(A.2.10) High 4:4:4 Predictive/Intra profile
                            // //Only I, P, B slice types may be present.
      || profile_idc == 44  // A.2.11 CAVLC 4:4:4 Intra profile
      || profile_idc == 83 // G.10.1.2.1 Scalable Constrained High profile (SVC)
                           // //Only I, P, EI, and EP slices shall be present.
    ...
    this->separate_colour_plane_flag =
        0; // When separate_colour_plane_flag is not present, it shall be
           // inferred to be equal to 0.
    if (this->chroma_format_idc == 3) {
      this->separate_colour_plane_flag =
          bs.readUn(1); // separate_colour_plane_flag 0 u(1)
    }

    this->bit_depth_luma_minus8 =
        gb.get_ue_golomb(bs); // bit_depth_luma_minus8 0 ue(v)
    this->bit_depth_chroma_minus8 =
        gb.get_ue_golomb(bs); // bit_depth_chroma_minus8 0 ue(v)
    this->qpprime_y_zero_transform_bypass_flag =
        bs.readUn(1); // qpprime_y_zero_transform_bypass_flag 0 u(1)
    this->seq_scaling_matrix_present_flag =
        bs.readUn(1); // seq_scaling_matrix_present_flag 0 u(1)
  ...
  this->log2_max_frame_num_minus4 =
      gb.get_ue_golomb(bs); // log2_max_frame_num_minus4 0 ue(v)
  ...
  if (this->pic_order_cnt_type == 0) {
    this->log2_max_pic_order_cnt_lsb_minus4 =
        gb.get_ue_golomb(bs); // log2_max_pic_order_cnt_lsb_minus4 0 ue(v)
  } else if (this->pic_order_cnt_type == 1) {
    this->delta_pic_order_always_zero_flag =
        bs.readUn(1); // delta_pic_order_always_zero_flag 0 u(1)
    this->offset_for_non_ref_pic =
        gb.get_se_golomb(bs); // offset_for_non_ref_pic 0 se(v)
    this->offset_for_top_to_bottom_field =
        gb.get_se_golomb(bs); // offset_for_top_to_bottom_field 0 se(v)
    this->num_ref_frames_in_pic_order_cnt_cycle =
        gb.get_ue_golomb(bs); // num_ref_frames_in_pic_order_cnt_cycle 0 ue(v)

    for (i = 0; i < (int32_t)this->num_ref_frames_in_pic_order_cnt_cycle; i++) {
      this->offset_for_ref_frame[i] =
          gb.get_se_golomb(bs); // offset_for_ref_frame[ i ] 0 se(v)
    }
  }

  this->max_num_ref_frames = gb.get_ue_golomb(bs); // max_num_ref_frames 0 ue(v)
  this->gaps_in_frame_num_value_allowed_flag =
      bs.readUn(1); // gaps_in_frame_num_value_allowed_flag 0 u(1)
  this->pic_width_in_mbs_minus1 =
      gb.get_ue_golomb(bs); // pic_width_in_mbs_minus1 0 ue(v)
  this->pic_height_in_map_units_minus1 =
      gb.get_ue_golomb(bs); // pic_height_in_map_units_minus1 0 ue(v)
  this->frame_mbs_only_flag = bs.readUn(1); // frame_mbs_only_flag 0 u(1)

  if (!this->frame_mbs_only_flag) {
    this->mb_adaptive_frame_field_flag =
        bs.readUn(1); // mb_adaptive_frame_field_flag 0 u(1)
  }

  this->direct_8x8_inference_flag =
      bs.readUn(1);                         // direct_8x8_inference_flag 0 u(1)
  this->frame_cropping_flag = bs.readUn(1); // frame_cropping_flag 0 u(1)

  if (this->frame_cropping_flag) {
    this->frame_crop_left_offset =
        gb.get_ue_golomb(bs); // frame_crop_left_offset 0 ue(v)
    this->frame_crop_right_offset =
        gb.get_ue_golomb(bs); // frame_crop_right_offset 0 ue(v)
    this->frame_crop_top_offset =
        gb.get_ue_golomb(bs); // frame_crop_top_offset 0 ue(v)
    this->frame_crop_bottom_offset =
        gb.get_ue_golomb(bs); // frame_crop_bottom_offset 0 ue(v)
  }

  this->vui_parameters_present_flag =
      bs.readUn(1); // vui_parameters_present_flag 0 u(1)
  ...
  //---------------------------
  if (m_vui.max_num_reorder_frames == -1) {
    // When the max_num_reorder_frames syntax element is not present
    // max_num_reorder_frames shall be in the range of 0 to
    // max_dec_frame_buffering, inclusive If profile_idc is equal to 44, 86,
    // 100, 110, 122, or 244 and constraint_set3_flag is equal to 1, the value
    // of max_num_reorder_frames shall be inferred to be equal to 0.
    if ((profile_idc == 44 || profile_idc == 86 || profile_idc == 100 ||
      ...
      m_vui.max_num_reorder_frames =
          MaxDpbFrames; // 此处的MaxDpbFrames就代表了码流中是否包含B帧，如果MaxDpbFrames>=2，则表示码流中含有B帧
    }
  }
...
int CH264SPS::get_h264_profile(char str[100]) {
  switch (profile_idc) {
  case 66: {
    sprintf(str, "H264 Baseline (profile=%d, level=%.1f)", profile_idc,
            level_idc / 10.0);
    break;
  }
  case 77: {
    sprintf(str, "H264 Main (profile=%d, level=%.1f)", profile_idc,
            level_idc / 10.0);
    break;
  }
  case 88: {
    sprintf(str, "H264 Extended (profile=%d, level=%.1f)", profile_idc,
            level_idc / 10.0);
    break;
  }
  case 100: {
    sprintf(str, "H264 High (profile=%d, level=%.1f)", profile_idc,
            level_idc / 10.0);
    break;
  }
  case 110: {
    sprintf(str, "H264 High 10 (profile=%d, level=%.1f)", profile_idc,
            level_idc / 10.0);
    break;
  }
  case 122: {
    sprintf(str, "H264 High 4:2:2 (Intra) (profile=%d, level=%.1f)",
            profile_idc, level_idc / 10.0);
    break;
  }
  case 244: {
    sprintf(str, "H264 High 4:4:4 (Predictive/Intra) (profile=%d, level=%.1f)",
            profile_idc, level_idc / 10.0);
    break;
  }
  default: {
    sprintf(str, "H264 Unkown (profile=%d, level=%.1f)", profile_idc,
            level_idc / 10.0);
    break;
  }
  }
